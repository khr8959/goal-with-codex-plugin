---
description: ユーザーの依頼内容を分析し、/goal-dual:run を使うべきかを提案する。実装・コミットは一切行わない。
argument-hint: '<依頼内容>'
...
ユーザーの依頼を分析して `/goal-dual:run` を使うべきかを提案します。
...
/goal-dual:run を使うべきかを判定し、推奨コマンドを提示します。
```
...
推奨コマンド:
  /goal-dual:run <suggested_goal>

※ goal-dual:run はゴール達成まで実装・テスト・評価を自動で繰り返します。
...
判定: ℹ️ /goal-dual:run は不要と判断しました
確信度: <confidence を % 表示>
リスク: <risk>
...
推奨コマンド:
  /goal-dual:run ログイン後にユーザー情報を表示できるようにする
