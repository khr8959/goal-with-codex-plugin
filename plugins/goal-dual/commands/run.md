...
| 変数名 | デフォルト | 説明 |
|--------|-----------|------|
| `GOAL_DUAL_WIP_COMMITS` | `0`（無効） | `1` にするとイテレーション途中の WIP commit を作成する。 |
| `GOAL_DUAL_FINAL_COMMIT` | `0`（無効） | `1` にすると完了時の commit を作成する。 |
| `GOAL_DUAL_STAGNATION_THRESHOLD` | `3` | 同一 verdict が何回連続すると STOP_STAGNANT と判定するかの閾値。 |
| `GOAL_DUAL_SCOPE_MODE` | `enforce` | `advisory` にすると変更禁止パス違反を警告のみにする。 |
| `GOAL_DUAL_ALLOW_HIGH_RISK` | `0` | `1` にすると Codex Work の `risk=high` でも自動続行候補にする。 |
...
- commit は既定で作成しない。ユーザーが `GOAL_DUAL_WIP_COMMITS=1` または `GOAL_DUAL_FINAL_COMMIT=1` を明示した場合のみ作成する
- Claude と Codex を人間同士のように長文会話させない。作業契約・作業結果・評価結果は短い typed packet / JSON / event として扱う
...
`GOAL_DUAL_SCOPE_MODE=enforce`（既定）の場合、run-loop.sh は各イテレーションで `scope-check.sh` を呼び、
`scope_deny`（変更禁止パス）への変更を検知すると commit 前に `stop_reason=STOP_SCOPE` で停止する
（`advisory` に明示変更した場合のみ警告だけでブロックしない）。
...
`pass` なら run-loop.sh が必要に応じて pass commit を作成し（既定では作成しない）
`completed=true` / `stop_reason=COMPLETE` を設定、
...
イテレーション数: <N>
ブランチ: <branch>
次のステップ: 差分と final-review.md を確認し、問題なければ手動で commit / push / PR 作成
